...
public class DimensionalEnergySiphonerMK2Block extends HorizontalBlock {
  ...
  public void addInformation(ItemStack stack, IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
    tooltip.add(new TranslationTextComponent("message:dimensional_energy_siphoner_mk2",
        Integer.toString(ReTCoConfig.machines.dimensionalEnergySiphonerMK2Generate.get())).mergeStyle(TextFormatting.GRAY));
  }
  ...
  public TileEntity createTileEntity(final BlockState state, final IBlockReader world) {
    return ReTCoTileEntityTypes.DIMENSIONAL_ENERGY_SIPHONER_MK2.get().create();
  }
  ...
  public int getComparatorInputOverride(BlockState blockState, World worldIn, BlockPos pos) {
    final TileEntity tileEntity = worldIn.getTileEntity(pos);
    if (tileEntity instanceof DimensionalEnergySiphonerMK2TileEntity)
      return ReTCoModUtil.calcRedstoneFromEnergyStorage(((DimensionalEnergySiphonerMK2TileEntity) tileEntity).energy);
