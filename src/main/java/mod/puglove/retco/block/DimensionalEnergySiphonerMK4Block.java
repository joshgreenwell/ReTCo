...
public class DimensionalEnergySiphonerMK4Block extends HorizontalBlock {
  public DimensionalEnergySiphonerMK4Block(final Properties properties) {
    super(properties);
  ...
  public void addInformation(ItemStack stack, IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
    tooltip.add(new TranslationTextComponent("message:dimensional_energy_siphoner_mk4",
        Integer.toString(ReTCoConfig.machines.dimensionalEnergySiphonerMK4Generate.get())).mergeStyle(TextFormatting.GRAY));
  }
  ...
  public TileEntity createTileEntity(final BlockState state, final IBlockReader world) {
    return ReTCoTileEntityTypes.DIMENSIONAL_ENERGY_SIPHONER_MK4.get().create();
  }
  ...
  public ActionResultType onBlockActivated(final BlockState state, final World worldIn, final BlockPos pos,
    ...
    if (!worldIn.isRemote) {
      ...
      if (tileEntity instanceof DimensionalEnergySiphonerMK4TileEntity)
        NetworkHooks.openGui((ServerPlayerEntity) player, (DimensionalEnergySiphonerMK4TileEntity) tileEntity, pos);
    }
  ...
  public int getComparatorInputOverride(BlockState blockState, World worldIn, BlockPos pos) {
    final TileEntity tileEntity = worldIn.getTileEntity(pos);
    if (tileEntity instanceof DimensionalEnergySiphonerMK4TileEntity)
      return ReTCoModUtil.calcRedstoneFromEnergyStorage(((DimensionalEnergySiphonerMK4TileEntity) tileEntity).energy);
