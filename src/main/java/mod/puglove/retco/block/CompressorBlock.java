...
public class CompressorBlock extends HorizontalBlock {
  public CompressorBlock(final Properties properties) {
		super(properties);
	...
	public TileEntity createTileEntity(final BlockState state, final IBlockReader world) {
		...
		return ModTileEntityTypes.COMPRESSOR.get().create();
	}
	...
	public void onReplaced(BlockState oldState, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
		if (oldState.getBlock() != newState.getBlock()) {
			...
			if (tileEntity instanceof CompressorTileEntity) {
				final ItemStackHandler inventory = ((CompressorTileEntity) tileEntity).inventory;
	...
	public ActionResultType onBlockActivated(final BlockState state, final World worldIn, final BlockPos pos, final PlayerEntity player, final Hand handIn, final BlockRayTraceResult hit) {
		if (!worldIn.isRemote) {
			...
			if (tileEntity instanceof CompressorTileEntity)
				NetworkHooks.openGui((ServerPlayerEntity) player, (CompressorTileEntity) tileEntity, pos);
		}
	...
	public int getComparatorInputOverride(BlockState blockState, World worldIn, BlockPos pos) {
		final TileEntity tileEntity = worldIn.getTileEntity(pos);
		if (tileEntity instanceof CompressorTileEntity)
			return ItemHandlerHelper.calcRedstoneFromInventory(((CompressorTileEntity) tileEntity).inventory);
