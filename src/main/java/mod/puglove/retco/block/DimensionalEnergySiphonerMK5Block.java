...
public class DimensionalEnergySiphonerMK5Block extends HorizontalBlock {
  public DimensionalEnergySiphonerMK5Block(final Properties properties) {
    super(properties);
  ...
  public void addInformation(ItemStack stack, IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
    tooltip.add(new TranslationTextComponent("message:dimensional_energy_siphoner_mk5",
        Integer.toString(ReTCoConfig.machines.dimensionalEnergySiphonerMK5Generate.get())).mergeStyle(TextFormatting.GRAY));
  }
  ...
  public TileEntity createTileEntity(final BlockState state, final IBlockReader world) {
    return ReTCoTileEntityTypes.DIMENSIONAL_ENERGY_SIPHONER_MK5.get().create();
  }
  ...
  public ActionResultType onBlockActivated(final BlockState state, final World worldIn, final BlockPos pos,
    ...
    if (!worldIn.isRemote) {
      ...
      if (tileEntity instanceof DimensionalEnergySiphonerMK5TileEntity)
        NetworkHooks.openGui((ServerPlayerEntity) player, (DimensionalEnergySiphonerMK5TileEntity) tileEntity, pos);
    }
  ...
  public int getComparatorInputOverride(BlockState blockState, World worldIn, BlockPos pos) {
    final TileEntity tileEntity = worldIn.getTileEntity(pos);
    if (tileEntity instanceof DimensionalEnergySiphonerMK5TileEntity)
      return ReTCoModUtil.calcRedstoneFromEnergyStorage(((DimensionalEnergySiphonerMK5TileEntity) tileEntity).energy);
