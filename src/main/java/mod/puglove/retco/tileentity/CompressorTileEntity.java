...
public class CompressorTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {
  public static final int INPUT_SLOT = 0;
    ...
    ReTCoConfig.machines.compressorCapacity.get(),
    ReTCoConfig.machines.compressorMaxInput.get()
  );
	...
	public CompressorTileEntity() {
		super(ModTileEntityTypes.COMPRESSOR.get());
	}
	...
	public void tick() {
		...
		if (!result.isEmpty() && isInput(input)) {
			...
			if (canInsertResultIntoOutput) {
				...
				final int energySmeltCostPerTick = ReTCoConfig.machines.compressorCostPerTick.get();
	...
	public ITextComponent getDisplayName() {
		return new TranslationTextComponent(ModBlocks.COMPRESSOR.get().getTranslationKey());
	}
	...
	public Container createMenu(final int windowId, final PlayerInventory inventory, final PlayerEntity player) {
		return new CompressorContainer(windowId, inventory, this);
	}
