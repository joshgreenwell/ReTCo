...
public class CompressorContainer extends Container {
  public final CompressorTileEntity tileEntity;
	 ...
	 */
	public CompressorContainer(final int windowId, final PlayerInventory playerInventory, final PacketBuffer data) {
		this(windowId, playerInventory, getTileEntity(playerInventory, data));
	}

	/**
	 * Constructor called logical-server-side from {@link CompressorTileEntity#createMenu}
	 * and logical-client-side from {@link #CompressorContainer(int, PlayerInventory, PacketBuffer)}
	 */
	public CompressorContainer(final int windowId, final PlayerInventory playerInventory, final CompressorTileEntity tileEntity) {
		super(ModContainerTypes.COMPRESSOR.get(), windowId);
		...
		this.addSlot(new SlotItemHandler(tileEntity.inventory, CompressorTileEntity.INPUT_SLOT, 56, 35));
		this.addSlot(new SlotItemHandler(tileEntity.inventory, CompressorTileEntity.OUTPUT_SLOT, 116, 35));
	...
	private static CompressorTileEntity getTileEntity(final PlayerInventory playerInventory, final PacketBuffer data) {
		Objects.requireNonNull(playerInventory, "playerInventory cannot be null!");
		Objects.requireNonNull(data, "data cannot be null!");
		final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
		if (tileAtPos instanceof CompressorTileEntity)
			return (CompressorTileEntity) tileAtPos;
	...
	public boolean canInteractWith(@Nonnull final PlayerEntity player) {
		return isWithinUsableDistance(canInteractWithCallable, player, ModBlocks.COMPRESSOR.get());
	}
}
